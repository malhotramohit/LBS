...
public class LMSTest {
	...
	public boolean loanBook(double d,int bookId)
	{
		...
		if(!custlist.isEmpty())
		{
			for(int i=0;i<custlist.size();i++)
			{
				...
				{
					if(custlist.get(i)!=null && custlist.get(i).getCustId()==d && (custlist.get(i).getBkl()).get(j).getStatus().equals("available"))
						c++;
				}
			}
		}
		...
		if(!custlist.isEmpty() && c<5)
		{
			for(int i=0;i<custlist.size();i++)
			{
				for(int j=0;j<(custlist.get(i).getBkl()).size();j++)
				{
				if(custlist.get(i)!=null && custlist.get(i).getCustId()==d && (custlist.get(i).getBkl()).get(j).getBookId()==bookId)
				{
	 ...
	 * @param bookList
	 * @return
	 * usage of collection.sort for wrapper class of primitive data types
	 */
	 ...
	 * @param bookList
	 * @return
	 * usage of collection.sort for object by implementing Comparable <obj> interface and override compareTo(obj arg0)
	 */
